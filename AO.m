...
% 
% where t is a hyperparameter controlled through a separate gradient
% descent routine.
%  
...
while iterate
    ...
    %   aopt.Q = Q + diag(rescale(erx(1:length(Q))));
    %end
...
function plot_hyper(x)

% if hypertune; plot_hyper(params.hyper_tau); end

s(1) = subplot(4,3,12);

plot(spm_vec(x),'w:','linewidth',3); hold on;
grid on;
...
switch lower(method)
    case {'free_energy','fe','freeenergy','logevidence'};
        ...
        case {'exp' 'ec' 'exp_cost'}
             t  = .5;
             fc = @(t) t*exp( (1./t)*sum( (Y-y).^2) );
