...
%                                         + cos(2*pi*x2))) + exp(1);
%
%     [X,F] = AO(@ackley_fun,[3 .5],[1 1]/128,0,[],[],[],1e-13,0,0,2,0,'fe')
%
% To do:
...
while iterate
    ...
    %
    % thus, if df0[i] > 0 then [ >p[i] == >e ] and [ <p[i] == <e ]
    %       if df0[i] < 0 then [ >p[i] == <e ] and [ <p[i] == >e ]
    ...
    while improve
        ...
        if StepMethod == 1
            % continue the ascent / descent (as per CA Rasmussen)
