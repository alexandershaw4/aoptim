function [EP,F,CP,History] = AO_DCMb(P,DCM,niter,mimo,method)
...
switch lower(method)
    ...
    [X,F,CP,History]  = AObayes(@fakeDM,p(:),c,DCM.xY.y,niter,12*4,[],1e-3,1e-12,mimo,2,0,'sse');
    case 'fe'
    % minimise free energy:
    fprintf('Minimising Free-Energy\n');
    [X,F,CP,History]  = AObayes(@fakeDM,p(:),c,DCM.xY.y,niter,12*4,[],-inf,1e-12,mimo,2,0,'fe');
    %[X,F,CP,History]  = AOm(@fakeDM,p(:),c,DCM.xY.y);
    case 'logevidence'
    fprintf('Minimising -[log evidence]\n');
    [X,F,CP,History]  = AObayes(@fakeDM,p(:),c,DCM.xY.y,niter,12*4,[],-inf,1e-12,mimo,2,0,'logevidence');
end
